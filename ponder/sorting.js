...
let simpleSort = simpleList.slice().sort(function(a, b) {
...
let filteredHikes = hikes.filter(function(hike) {
  return hike.name.toLowerCase().includes(hikeQuery.
    toLowerCase()) || hike.description.toLowerCase().includes(hikeQuery.toLowerCase())
    || hike.tags.some(tag => tag.toLowerCase().includes(hikeQuery.toLowerCase()));
